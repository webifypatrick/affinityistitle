...
namespace Affinity
{
    public partial class Documents : PageBase
    {
        ...
        protected string extractPdfText(string path)
        {
            ...
            try
            {
                ...
                {
                    sb.Append(iTextSharp.text.pdf.parser.PdfTextExtractor.GetTextFromPage(pdfreader, i, new iTextSharp.text.pdf.parser.SimpleTextExtractionStrategy()));
                }
            }
            catch (Exception ex)
            {
        ...
        protected string extractLegalDescription(string name)
        {
            string ret = "";
            try
            {
                ...
                if (pos > -1)
                {
                    string str = pdftext.Substring(pos + 18);
                    pos = str.IndexOf("Affinity Title Services, LLC");
                    if (pos > -1)
                    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            */
            ...
            if (id > 0)
            {
                ...
                Attachments atts = order.GetAttachments();
                ...
                foreach (Attachment att in atts)
                {
                    if(att.AttachmentPurpose.Code.Equals("Committment"))
                    {
                        ...
                        {
                            //Response.Write(legaldescription + "<Br>");
                            //break;
                        }
                    }
                }
                ...
                int filecount = 0;
                ((Affinity.MasterPage) this.Master).SetLayout("Documents", MasterPage.LayoutStyle.ContentOnly);
                ...
                ("SELECT * FROM agent_examination WHERE aef_order_id = " + id.ToString() + " ORDER BY aef_id DESC LIMIT 1", mysqlCon))
                    ...
                    try
                    {
                        ...
                        {
                            using (MySqlDataReader reader = this.phreezer.ExecuteReader("select * from `taxing_district` t where t.taxing_district = '" + GrantorCity + "'"))
                            {
                                if (reader.Read())
                                {
                    ...
                    // Put in a legal description if there is one
                    if(!legaldescription.Equals(""))
                    {
                        contents = contents.Replace("<w:fldChar w:fldCharType=\"begin\"/></w:r><w:r><w:rPr><w:szCs w:val=\"20\"/></w:rPr><w:instrText xml:space=\"preserve\"> MACROBUTTON  AcceptAllChangesShown \"[Click here and type Legal Description of Property]\" </w:instrText></w:r><w:r><w:rPr><w:szCs w:val=\"20\"/></w:rPr><w:fldChar w:fldCharType=\"end\"/>", "<w:t>" + legaldescription + "</w:t>");
                    }
